				...
				try
				{
					...
					{//Set NS fields based on integration inputs
						...
						else
						{
							...
							else if ('CNS' in Fldval)
							{
								var Access_Params;
								if (Fldval.$==null || Fldval.$=='')
									{
									NSRecord.setFieldValue (Fldnm, '');	
									}
								else
									{
								Access_Params=Fldval.$.split(".");
								NSRecord.setFieldValue (Fldnm, INTEGRATION_CONSTANTS[Access_Params[0]][Access_Params[1]]);
									}
							}
		...
		for(var OLineFidx=0;OLineFidx<Object.keys(Olineitem).length;OLineFidx++)
    	 {
			...
			else if ('CNS' in Fldval)
			{
				...
				if (Fldval.$==null || Fldval.$=='')
				{
					NSR.setCurrentLineItemValue (NSsublistNm,Fldnm,  '');	
				}
				else
					{
				Access_Params=Fldval.$.split(".");
				NSR.setCurrentLineItemValue (NSsublistNm,Fldnm, INTEGRATION_CONSTANTS[Access_Params[0]][Access_Params[1]]);
					}
			}
			}
