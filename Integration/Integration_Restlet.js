		...
		else
		{
			if ('Lookups' in request)
			{
				...
				for (Clk in request.Lookups)
				{
					...
					// nlapiLogExecution('DEBUG', 'Check point 2', 'check point 2');
					...
					// nlapiLogExecution('DEBUG', 'Check point 2', 'check point pass');
				}
...
function GetvaluefromArr(Id,Arr,Binary_S_mode)
{
	...
	if (Arr==null || (typeof Arr[0] == 'undefined')) return null;
	
//	nlapiLogExecution('DEBUG', 'Check point 4.1', 'check point 4.1');
//	nlapiLogExecution('DEBUG', 'Check point 4.1 -- Arr', JSON.stringify(Arr));
	if (Binary_S_mode) return GetvaluefromArr_BS(Id,Arr);
	
//	nlapiLogExecution('DEBUG', 'Check point 4.1', 'check point pass');
...
{
//	nlapiLogExecution('DEBUG', 'entered binary search',  'entered binary search');
//	nlapiLogExecution('DEBUG', 'bs array', JSON.stringify(items));
	    var startIndex  = 0,
	        stopIndex   = items.length - 1,
	        middle      = Math.floor((stopIndex + startIndex)/2);
	    
//	    nlapiLogExecution('DEBUG', 'stop : middle : value', stopIndex + " :" + middle + ":"+ value);
...
//	        nlapiLogExecution('DEBUG', 'After Recalc ---> stop : middle : value : startIndex', stopIndex + " :" + middle + ":"+ value + ":" + startIndex);

	    }
	    
//	    nlapiLogExecution('DEBUG', 'exit binary search',  'exit search value middle:' + middle);
			...
			for (Ridx=0;Ridx<Records.length;Ridx++)
			{
				try
				{
					...
					{ //locate internal id from array incase not available from broker
						 nlapiLogExecution('DEBUG', 'Check point 3', 'check point 3');
						...
						// nlapiLogExecution('DEBUG', 'Check point 3', 'check point pass');
					}
					...
					{//Set NS fields based on integration inputs
						...
						else
						{
							...
							/*else if ('Function' in Fldval)
							{
							    var funcnm=Fldval.Funcnm;
							     get parmeteraes  ,max 3 
							    var param1=fldval.Parm1;
							    var param2=fldval.Parm2;
							    var param3=fldval.Parm3;
							    var paramcnt=Fldval.ParamCnt;
							 
							    if (paramcnt==1)
							    { //single parameter function
							    		if ((param1!=null) || (param1!=''))
							    			NSRecord.setFieldValue (Fldnm, eval(funcnm+"("+param1+")"));
							    }
							    else if (paramcnt==2)
							    	{
							    	//single parameter function
						    		if (((param1!=null) || (param1!='')) && ((param2!=null) || (param2!='')))
						    			NSRecord.setFieldValue (Fldnm, eval(funcnm+"("+param1+","+param2+")"));
							    	}
							    else if (paramcnt==3)
						    	{
						    	//single parameter function
					    		if (((param1!=null) || (param1!='')) && ((param2!=null) || (param2!='')) && ((param3!=null) || (param3!='')))
					    			NSRecord.setFieldValue (Fldnm, eval(funcnm+"("+param1+","+param2+","+param3+")"));
						    	}
							...
							else if ('Lkname' in Fldval)
							{
								...
								nlapiLogExecution('DEBUG', 'Check point 4', 'check point 4');
							...
							//	nlapiLogExecution('DEBUG', 'Check point 4', 'check point pass');
							}						
							else if ('SublistKey' in Fldval)
							{
									...
									if ('SublistKey2' in Fldval)
										SublistKey2=Fldval.SublistKey2;
									else
										SublistKey2=null;
									
									SublistStatus=Integrate_Sublist(NSRecord,Fldval.List,Fldval.SublistKey,Fldval.NSsublistNm,SublistKey2,UpdateFlag);		
...
function Integrate_Sublist(NSR,Sublist,Key,NSsublistNm,Key2,UpdateFlag)
{
	...
	for(OSublistIdx=0;OSublistIdx<ArrSublist.length;OSublistIdx++)
	{
		...
		var Linefound=false;
		var KeyCmp="(OSublistkey==NSR.getLineItemValue (NSsublistNm, Key, NSSublistIdx))";
		var SecondKeyVal;
		
		if (Key2!=null && Key2!='')
			{
			KeyCmp=KeyCmp + " && (ArrSublist[OSublistIdx][Key2]==NSR.getLineItemValue (NSsublistNm, Key2, NSSublistIdx))";
			}
		...
		/*	else if ('Function' in Fldval)
			...
			//	nlapiLogExecution('DEBUG', 'Check point 5', 'check point 5');
				
				var LkRes=GetvaluefromArr(Fldval.$,Support_Lookups[Fldval.Lkname],true);
			//	nlapiLogExecution('DEBUG', 'Check point 5', 'check point pass');
